...
describe('detail reducer', ()=>{
    ...
    describe('#detail', ()=>{
        ...
        it('should return the `INSERT_UPDATE_POST` set input value', ()=>{
            expect(detail({},{type: INSERT_UPDATE_POST, model:{ "input": 222 } })).toMatchSnapshot()
        })
    ...
    describe('#error', ()=>{
        ...
        ('should return the `%p` set %s input value', (type, message)=>{
            expect(error(null, { type, message })).toMatchSnapshot()
            expect(error(undefined, { type, message })).toBe(message)
        })
    })
})